...
def cria_expediente():
    ...
    driver.find_element(
        'xpath', "//a[contains(text(),'Expediente de verificação fiscal')]").click()
    driver.find_element('name', 'complemento_do_assunto').send_keys(
        'Não localização - Programa Nos Conformes')
    driver.find_element('name', 'interessado_nome').send_keys(osf['Nome'])
    driver.find_element('name', 'interessado_cnpj').send_keys(osf['CNPJ'])
    driver.find_element('name', 'interessado_ie').send_keys(osf['IE'])
    driver.find_element('name',
                        'interessado_logradouro').send_keys(osf['Logradouro'])
    driver.find_element('name', 'interessado_numero').send_keys(osf['Número'])
    # driver.find_element('name', 'interessado_endereco_complemento').send_keys('')
    # driver.find_element('name', 'interessado_bairro').send_keys('')
    driver.find_element('name',
                        'interessado_municipio').send_keys(osf['Cidade'])
    driver.find_element('name', 'interessado_municipio_uf').click()
    driver.find_element('xpath', "//option[@value='SP']").click()
    driver.find_element('name', 'interessado_cep').send_keys(osf['CEP'])
    # driver.find_element('name', 'ver_doc_pdf').click()
...
def formulario_205p():
    ...
    if está_na_tela_processo():
        ...
        driver.find_element('xpath', "//option[@value='SP']").click()
        driver.find_element('name', 'interessado_cep').send_keys(osf['CEP'])
        driver.find_element('name', 'numero_osf').send_keys(osf['OSF'])
...
def junta_fotos(verifica=True):
    ...
    if está_na_tela_processo():
        ...
        driver.find_element('name', "especie").click()
        ...
        driver.find_element('name', "conferencia").click()
        driver.find_element('xpath',
                            "//option[@value='Documento Original']").click()

        # Preenche "Descrição"
        driver.find_element('name', "outros").send_keys("Fotografia")
    else:
